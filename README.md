...
Inbox is an independent Ash-native task and job queue built on Oban. It combines
a reusable task lifecycle with AshOban's typed host-action execution. Applications
keep their own Ash resources, policies, and Oban instance; `Inbox.Task` adds the
shared behavior without creating another job engine.

## Installation

Inbox is available independently from Git, or through the AshLotus distribution:

```elixir
{:inbox, github: "vintrepid/inbox", tag: "v0.1.0-alpha.1"}
```

When using AshLotus, declare only `ash_lotus`; it supplies Inbox. Pre-1.0 APIs
may change between releases. Git pins make builds reproducible, not interfaces
permanent.

## Status

`0.1.0-alpha.1` is experimental. It provides standalone task lifecycle,
...
hosts explicitly select which Ash actions are executable. Inbox does not yet
...
semantic journal. No worker starts merely because Inbox is a dependency, and
...
Inbox supplies Ash 3.33+, AshStateMachine 0.2.13+, Oban 2.23+, and AshOban
...
Inbox does not supply a repository or migrations. Set Ash's string counting
...
such as `simple_sat` or `picosat_elixir`. Inbox's authorization tests use a
...
For transport retries, `Inbox.Task.complete_by_request_key/4` and
`Inbox.Task.cancel_by_request_key/4` take the host resource module, request key,
...
policy, not an Inbox default. Real hosts should grant a dedicated worker only its
...
serialize credentials into job arguments. Inbox installs no policy bypass.

Use the host's existing Oban child with its repository and the `:inbox` queue
